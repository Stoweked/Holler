...
const AmountInput = forwardRef<HTMLInputElement, AmountInputProps>(
  (
    { amount, setAmount, initialBalance, flowType },
    ...
    if (length > 9 && fontSize !== "2rem") setFontSize("2rem");
    else if (length > 6 && fontSize !== "2.5rem") setFontSize("2.5rem");
    else if (length <= 6 && fontSize !== "3.5rem") setFontSize("3.5rem");
    ...
    return (
      <Stack align="center" gap="xs">
        <NumberInput
          w="100%"
          ref={ref} // ✅ ref is forwarded from parent
          form={undefined} //prevent form submission on "Enter"
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              e.preventDefault(); // prevent form submit
              (e.currentTarget as HTMLInputElement).blur(); // close keyboard
            }
          }}
          value={amount}
...
AmountInput.displayName = "AmountInput"; // avoid forwardRef warning
